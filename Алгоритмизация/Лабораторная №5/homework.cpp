...
int main() {
    ...
    // Включаем поддержку локали вашей Windows 10
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("stockholm_drive.graphml");
    //  print_edges(edges);
    // 2. Задаём названия улиц для начала и конца маршрута
    Coord start, end;
    string name;
    std::tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Hamngatan";      // Название улицы для старта (пример)
    string end_street_query = "Salagatan";      // Название улицы для финиша (пример)
