...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    float mathX = (mousePos.x - 400) / 0.4f; // Масштаб 0.4 по X
                    float mathY = -(mousePos.y - 300) / 0.2f; // Масштаб 0.2 по Y
                    ...
                    std::string position;
                    int red = abs(mathX) - mathX;
                    int blue = -abs(mathX);
                    if (mathY < blue) {
                        position = "Area 1";
                    } else if ((mathX > 0) and (mathY < red)) {
                        position = "Area 4";
                    } 
                    else if ((mathX < 0) and (mathY < red)) {
                        position = "Area 3";
        ...
        drawGraph(window, [](float x) { return abs(x - abs(x));}, -10, 10, 30, 10, sf::Color::Red);
