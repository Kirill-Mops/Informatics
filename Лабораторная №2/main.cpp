...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    float mathX = (mousePos.x - 400) / 40.0f; // Масштаб 40 по X
                    float mathY = -(mousePos.y - 300) / 20.0f; // Масштаб 20 по Y
                    ...
                    std::string position;
                    float red = abs(mathX) - mathX;
                    float blue = -abs(mathX);
                    if ((mathY == red) or (mathY == blue)) {
                        position = "Border";
                    }
                    else if (mathY < blue) {
                        position = "Area 1";
        ...
        drawGraph(window, [](float x) { return -abs(x); }, -10, 10, 40, 20, sf::Color::Blue);

        // 5 _   Отрисовка графика y2 = x * x - 5 (Замените на ваш график)
        drawGraph(window, [](float x) { return abs(x - abs(x)); }, -10, 10, 40, 20, sf::Color::Red);
